...
from recommendations import (
    ...
    suggest_hashtags,  # Removed suggest_polls_to_vote
...
def emit_new_activity_event(activity_log):
    ...
    ):  # Check if activity_log and its user attribute exist
        app.logger.error(
    ...
    friends_of_actor = (
        actor.get_friends()
    )  # Assuming get_friends() is a method on User model
    if friends_of_actor:
...
)  # Added OnThisDayResource endpoint
...
def user_profile(username):
    ...
    current_user_id = session.get(
        "user_id"
    )  # Get current_user_id for friendship status
    if current_user_id:
    ...
    if current_user_id and current_user_id != user.id:
        ...
        if existing_friendship:
            if existing_friendship.status == "accepted":
                ...
                if (
                    existing_friendship.user_id == current_user_id
                ):  # Current user's request was rejected by 'user'
        ...
        if file and allowed_file(file.filename):
            ...
            if user_id:
                ...
                if user:
                    ...
                    ):  # Avoid duplicates if re-uploading same name
...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user.password_hash, password_candidate):
            session["logged_in"] = (
                True  # Can keep for compatibility or remove if only user_id is used
            )
            ...
            return redirect(
                url_for("hello_world")
            )  # Or a dashboard page if you create one
        else:
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        if (
            not new_email
        ):  # Basic check, more complex validation (e.g., regex) could be added
        ...
        if new_email != user.email:
            ...
            if existing_email_user:
                ...
                current_form_data = {
                    ...
                    "email": user.email,
                }  # Keep current email if new one is bad
                return render_template("edit_profile.html", user=current_form_data)
            user.email = new_email

        user.bio = (
            new_bio  # Update bio, no complex validation for now, just strip whitespace
        )
    ...
    if request.method == "POST":
        ...
        try:
            activity = UserActivity(
                ...
                ),  # Use _external=True for full URL
            )
            db.session.add(activity)
            db.session.commit()
...
def blog():
    ...
    )  # Pass snippet to template


@app.route(
    "/blog/post/<int:post_id>"
)  # Accepts optional series_id for navigation context
            ...
            if listeners:  # Check if there are actual listeners after fetching the list
                ...
                for q in listeners:
                    try:
                        ...
                        Exception
                    ) as e:  # Catch broad exceptions, e.g., queue.Full or if q is no longer valid
                        app.logger.error(
    ...
    if post_id in app.sse_listeners:
        ...
        for q in app.sse_listeners[post_id]:
            try:
                q.put_nowait(
                    sse_event
                )  # Use put_nowait to avoid blocking if queue is full (though default is unbounded)
...
def like_post(post_id):
    ...
    if not existing_like:
        ...
        try:
            ...
            Exception
        ) as e_like:  # Catch error from liking post or notification logic
            db.session.rollback()
            app.logger.error(
        ...
        try:
            ...
            if post_id in app.sse_listeners and q in app.sse_listeners[post_id]:
                ...
                if not app.sse_listeners[
                    post_id
                ]:  # If list is empty, remove post_id key
...
def send_message(receiver_username):
    ...
    if request.method == "POST":
        ...
        socketio.emit(
            "new_direct_message",
            message_payload,
            room=f"user_{new_message_db.receiver_id}",
        )
...
def handle_join_room_event(data):
    app.logger.info(
        f"User {session.get('username', 'Anonymous')} joined room: {data['room']}"
    )
    join_room(data["room"])
...
def handle_send_group_message_event(data):
    ...
    if not user_id:
        ...
        emit(
            "error_event", {"message": "Authentication required to send messages."}
    ...
    if not group_id:
        app.logger.error(
            f"User {username} (ID: {user_id}) tried to send group message without group_id."
        )
    ...
    if not message_content or not message_content.strip():
        app.logger.info(
            f"User {username} (ID: {user_id}) tried to send an empty message to group {group_id}."
        )
    ...
    try:
        ...
        app.logger.info(
            f"User '{username}' sent message to group {group_id}: '{message_content}' (not saved)"
        )

    except Exception as e:
        db.session.rollback()
        app.logger.error(
            f"Error sending (but not saving) group message for group {group_id} by user {user_id}: {e}"
        )
        emit(
            "error_event", {"message": "An error occurred while sending your message."}
        )
...
def handle_edit_post_content(data):
    ...
    if lock.user_id != user_id:
        ...
        if lock.expires_at.replace(tzinfo=timezone.utc) <= datetime.now(timezone.utc):
            ...
            emit(
                "edit_error",
                {
                    "message": "Post lock by another user has expired. Try acquiring the lock."
                },
                room=request.sid,
            )
        else:
    ...
    if lock.expires_at.replace(tzinfo=timezone.utc) <= datetime.now(
        ...
        emit(
            "edit_error",
            {
                "message": "Your lock on the post has expired. Please acquire a new lock."
            },
            room=request.sid,
        )
        return
    ...
    try:
        ...
        emit(
            "edit_error",
            {
                "message": "A server error occurred while saving changes. Please try again."
            },
            room=request.sid,
        )
...
def create_group():
    if request.method == "POST":
        ...
        new_group.members.append(
            current_user
        )  # SQLAlchemy handles the association table
...
def view_friends_list(username):
    ...
    friends_list = (
        user.get_friends()
    )  # This method should return a list of User objects
...
def edit_series(series_id):
    ...
    if request.method == "POST":
        ...
        )  # Redirect to edit_series to see changes and continue managing posts
    ...
    }  # Get IDs from the already ordered posts in the series
    available_posts = [
        post for post in user_posts if post.id not in posts_in_series_ids
    ]
...
def add_post_to_series(series_id, post_id):
    ...
    ):  # Ensure post is by the same author as the series
...
def remove_content_and_reject_flag(flag_id):
    ...
    if content_removed:
        ...
        )  # Redirect after flashing error if content not removed
...
def set_status():
    ...
    ):  # Should ideally not happen if @login_required is effective
        flash("User not found. Please log in again.", "danger")
        return redirect(url_for("login"))
