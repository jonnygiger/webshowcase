...
def suggest_posts_to_read(user_id, limit=5):
    ...
    for post_id in recommended_posts_with_ts.keys():
        ...
        if post:  # Ensure post exists
            ...
            if post_id in user_liked_post_ids:  # Exclude posts liked by current user
                continue
            if (
                post_id in user_commented_post_ids
            ):  # Exclude posts commented on by current user
                continue
    ...
    RECENCY_HALFLIFE_DAYS = (
        7  # For calculating recency score, e.g. score halves every 7 days
    )
    ...
    }  # Assumes Bookmark model is imported and User.bookmarks relationship exists
...
def suggest_trending_posts(user_id, limit=5, since_days=7):
    ...
    for post in posts_to_score:
        ...
        ):  # Only apply strong recency if within the window
...
def get_personalized_feed_posts(user_id, limit=20):
    ...
    )  # Assuming this method exists and returns User objects
            ...
            for post in posts_from_groups:
                ...
                if post.group_id:  # Check if group_id is set
                    group = Group.query.get(
                        post.group_id
                    )  # Potential N+1 if not careful, better to join if displaying many group posts
                    if group:
