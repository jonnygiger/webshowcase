...
from flask_login import (
    current_user, # Assuming flask_login's current_user will be used
    login_user,
    logout_user,
)
from sqlalchemy import or_

# Assuming db, User, Post etc. are available from social_app.models.db_models
# and extensions like socketio, login_manager are initialized in social_app/__init__.py
from .. import db, socketio, login_manager # Use .. for relative import from parent package
...
from .utils import (
    ...
    generate_activity_summary, allowed_shared_file # seed_achievements_command is CLI, move to run.py
)
...
# from ..services.notifications_service import new_post_sse_queues, broadcast_new_post # If needed directly in views

# Create a Blueprint for core views
# The name 'core' here is arbitrary but conventional for a core module.
# The first argument 'core' will be used as the prefix for route names, e.g., 'core.hello_world'
# template_folder and static_folder are removed as the app level ones are set correctly.
core_bp = Blueprint('core', __name__)
# Adjust template_folder and static_folder if your templates/static are not in the default locations
# relative to this blueprint's location. Given the project structure, they are likely at the root.
# If templates are in `social_app/templates` and static in `social_app/static`, adjust path.
# For a typical Flask structure where `templates` is at the root of the project (sibling to `run.py` and `social_app` dir):
# template_folder='../../templates', static_folder='../../static' might be needed if Flask can't find them.
# However, Flask usually looks for a `templates` folder in the application root or next to the blueprint file.
# If your `social_app` is the app root for Flask, then `../templates` might be right if `templates` is inside `social_app`.
# Let's assume templates & static are at the project root, so `../../templates` and `../../static` from `social_app/core/`

# Custom Jinja2 filter for nl2br - this should be registered on the app or blueprint
@core_bp.app_template_filter()
def nl2br_filter(value):
    return nl2br(value)


# Helper function for emitting activity events (moved from app.py)
# This needs access to `socketio` and `current_app.logger` (or just `app.logger` if app is passed)
# It also needs `db` and `User` model.
...
@core_bp.route("/")
def hello_world():
    featured_post = get_featured_post() # Assumes get_featured_post uses current_app context for db session
...
def user_profile(username):
    ...
    # Use flask_login.current_user if available and authenticated
...
@login_required
def todo():
    user_id = current_user.id # flask_login current_user
...
def upload_image():
    if request.method == "POST":
        ...
        if file.filename == "":
            ...
            return redirect(request.url)
        if file and allowed_file(file.filename): # allowed_file needs current_app context for config
            filename = secure_filename(file.filename)
            # current_app.config['UPLOAD_FOLDER'] should be used
...
def gallery():
    ...
    if os.path.exists(upload_folder):
        for filename in os.listdir(upload_folder):
            if allowed_file(filename): # allowed_file needs current_app context
...
def login():
    ...
    if request.method == "POST":
        ...
        user_obj = User.query.filter_by(username=username).first() # Renamed to avoid conflict
        if user_obj and check_password_hash(user_obj.password_hash, password_candidate):
            login_user(user_obj) # Use flask_login's login_user
            # session management is handled by flask_login
...
@login_required # Ensures user is logged in before trying to log out
def logout():
...
def upload_profile_picture():
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename): # allowed_file needs current_app
            filename = secure_filename(file.filename)
...
# This should be registered on the app itself, or on the blueprint using @blueprint.context_processor
@core_bp.context_processor
def inject_user_into_templates():
    # flask_login's current_user is already available in templates if LoginManager is initialized
        ...
        if post_author:
            ...
            if friends:
                ...
                if notifications_to_send:
                    try:
                        db.session.add_all(notifications_to_send) # More efficient
                        db.session.commit()
...
def edit_post(post_id):
    ...
    if request.method == "POST":
        ...
        db.session.commit()
        # SSE Event for post_edited (needs current_app for sse_listeners and logger)
...
@login_required
@moderator_required # Assuming moderator_required decorator is defined in utils
...
def add_comment(post_id):
    ...
    if post_id in current_app.sse_listeners:
        ...
        for q in current_app.sse_listeners[post_id]:
            try: q.put_nowait(sse_event)
...
def post_stream(post_id):
    # This SSE route needs current_app for sse_listeners and logger
    def event_stream():
        import queue # Keep import here as it's specific to this generator
        q = queue.Queue()
        if post_id not in current_app.sse_listeners:
            current_app.sse_listeners[post_id] = []
        current_app.sse_listeners[post_id].append(q)
        try:
            while True:
                try:
            ...
            if post_id in current_app.sse_listeners and q in current_app.sse_listeners[post_id]:
                current_app.sse_listeners[post_id].remove(q)
...
@login_required
def chat_page():
    return render_template("chat.html") # current_user is available from context processor
...
def bookmark_post(post_id):
    ...
    if existing_bookmark:
        ...
        db.session.commit()
        if user_id and new_bookmark: # Achievement check
...
def view_notifications():
    # This route might need to be user-specific if Notification model has a user_id
    # For now, showing all, similar to original app.py
...
# Route for testing notification generation (from app.py)
@core_bp.route("/trigger_notifications_test_only")
@login_required
def trigger_notifications_test_only():
    if current_app.debug:
        generate_activity_summary() # Needs current_app context if it uses app features
...
def view_group(group_id):
    ...
    chat_messages = [] # Temporarily disable chat
...
def create_group():
    if request.method == "POST":
        ...
        user_obj = db.session.get(User, user_id) # Renamed current_user to user_obj to avoid conflict
...
def join_group(group_id):
    group = Group.query.get_or_404(group_id)
    user_id = current_user.id
    user_obj = db.session.get(User, user_id) # Renamed
...
def send_friend_request(target_user_id):
    ...
    if existing_friendship:
        if existing_friendship.status == "pending": flash("Friend request already sent or received and pending.", "info")
            ...
            else: # target_user rejected it, current_user can send new one
    ...
    # SSE Notification for friend request (needs current_app for user_notification_queues and logger)
    sender_user_obj = db.session.get(User, current_user_id_val) # Renamed
    if sender_user_obj:
...
def accept_friend_request(request_id):
    ...
    if friend_request.status == "pending":
        ...
        # SSE Notification (needs current_app for user_notification_queues and logger)
        accepting_user_obj = db.session.get(User, current_user_id_val) # Renamed
...
def view_series(series_id):
    ...
    return render_template("view_series.html", series=series) # series.posts should be ordered by model definition
...
def edit_series(series_id):
    ...
    if request.method == "POST":
        ...
        if not title or not title.strip():
            ...
            # Re-populate for template render on error
...
@login_required
def live_feed():
    user_obj = db.session.get(User, current_user.id) # Renamed
...
def recommendations_view():
    ...
    suggested_hashtags_list = suggest_hashtags(user_id, limit=5) # Renamed
...
def flag_post(post_id):
    post = db.session.get(Post, post_id)
    if not post: return jsonify({"message": "Post not found"}), 404 # API-like response for consistency if this was one
    user_id = current_user.id
...
def flag_comment(comment_id):
    ...
    if not comment: return jsonify({"message": "Comment not found"}), 404 # API-like response
    user_id = current_user.id
...
def remove_content_and_reject_flag(flag_id):
    ...
    else: return redirect(url_for("core.moderation_dashboard")) # If content not removed, redirect, flag remains pending.
...
def share_file_route(receiver_username):
    ...
    if request.method == "POST":
        ...
        if file and allowed_shared_file(file.filename): # allowed_shared_file needs current_app
            ...
            if not extension and "." in true_original_filename: # Fallback
...
def user_notification_stream():
    # This SSE route needs current_app for user_notification_queues and logger
    current_user_id_val = current_user.id
    import queue # Keep import local to this generator function
    q = queue.Queue()
    ...
    current_app.user_notification_queues[current_user_id_val].append(q)
    ...
    def event_stream():
        try:
            ...
            if current_user_id_val in current_app.user_notification_queues:
                if q in current_app.user_notification_queues[current_user_id_val]:
                    current_app.user_notification_queues[current_user_id_val].remove(q)
...
# Removed /api/posts/stream as it was related to notifications_service.py and broadcast_new_post
# which will be handled differently or within notifications_service if it exposes an SSE endpoint.
# The login_required_socketio decorator and the more complex handle_connect SocketIO event
# are moved to core/events.py as they are SocketIO specific.
# The simpler handle_connect was a duplicate.
# The api_login route using JWT is an API concern, moved to api/routes.py.
# The context_processor inject_user is not strictly needed if using flask_login.current_user,
# but if kept, it should be registered on the app or blueprint. Included in this BP for now.
# emit_new_activity_event is kept here as it's called by various view functions in this file.
# It needs socketio, db, User, url_for, and current_app.logger.
# nl2br filter is registered on the blueprint using app_template_filter.
# All url_for calls have been updated to use the blueprint name, e.g., url_for('core.view_post', ...)
# Assumed flask_login.current_user is the primary way to get the current user.
# Replaced session.get('user_id') with current_user.id where appropriate.
# Replaced app.logger with current_app.logger.
# Replaced app.config with current_app.config.
# Relative imports like `from .. import db` are used for components from `social_app/__init__.py`.
# Imports from submodules like `from .utils import ...` are used for other parts of the core module.
# Imports from other top-level packages like `from ..services.achievements import ...` are used.
# seed_achievements_command is a CLI command, will be moved to run.py.
# generate_activity_summary is a background task, will be part of utils or services and called from scheduler in run.py.
# Note: The template_folder and static_folder for the Blueprint have been set assuming they are at the project root.
# This might need adjustment based on actual project structure.
# Example: `Blueprint('core', __name__, template_folder='../templates', static_folder='../static')`
# if templates/static are inside the `social_app` directory.
# Or `Blueprint('core', __name__, template_folder='templates')` if they are in `social_app/core/templates`.
# For now, using `../../templates` to point to project root from `social_app/core/`.
# This is often handled by Flask's auto-discovery if `templates` is in the app root.

# Registration of this blueprint on the app object (from create_app in __init__.py) will be like:
# from .core.views import core_bp as core_views_bp
# app.register_blueprint(core_views_bp)
# And if there's a url_prefix for these routes:
# app.register_blueprint(core_views_bp, url_prefix='/app') # Example prefix
# For now, no url_prefix is assumed for the core blueprint itself.
# emit_new_activity_event needs `url_for` which requires app context.
# If called outside request context (e.g. background task), ensure app context is pushed.
# However, in this file, it's called from view functions which have app context.
# The `socketio` object is imported from `social_app/__init__.py`.
# The `db` object is imported from `social_app/__init__.py`.
# `login_manager` is imported from `social_app/__init__.py`.
# `check_and_award_achievements` is imported from `social_app/services/achievements.py`.
# Recommendation functions are imported from `social_app/services/recommendations_service.py`.
# Utility functions like `allowed_file`, `login_required` are imported from `.utils`.
# All model imports are from `..models.db_models`.
# Flask's `current_user` (from `flask_login`) is used instead of `session.get('user_id')`.
# `app.sse_listeners` and `app.user_notification_queues` are accessed via `current_app`.
# `uuid` module was used for `upload_profile_picture`, ensured it's imported.
import uuid # For generating unique filenames in upload_profile_picture

# Make sure to register this blueprint in social_app/__init__.py's create_app function.
# e.g.:
# from .core import views as core_views
# app.register_blueprint(core_views.core_bp)

# Also, the custom Jinja filter nl2br needs to be registered.
# It's done above using @core_bp.app_template_filter().
# Alternatively, in create_app: app.jinja_env.filters['nl2br'] = nl2br_filter_function

# The `emit_new_activity_event` function was moved here. It needs access to `socketio`, `db`, `User`, `url_for`, `current_app.logger`.
# These should be available via imports or `current_app`.
# `url_for` needs blueprint name for local routes: `url_for('core.user_profile', ...)`
# If `_external=True` is used, it needs server name configured or request context.

# `get_featured_post` (from utils) will need access to `db` and `Post` model, and `current_app.logger`.
# `generate_activity_summary` (from utils) will need `db`, models, `current_app.logger`, and `current_app.last_activity_check_time`.
# `current_app.last_activity_check_time` would need to be initialized on the app object.
# This function is intended for the scheduler.

# `allowed_file` and `allowed_shared_file` (from utils) need `current_app.config`.
# `login_required` and `moderator_required` decorators (from utils) need `current_user`, `db`, `User`, `flash`, `redirect`, `url_for`.
# `url_for` within decorators should also use blueprint names e.g. `url_for('core.login')`.

# The file is quite long. It might be beneficial to split it further later,
# for example, by grouping related views into different files/blueprints (e.g., auth_views, post_views, user_views).
# For now, it's a direct move of route handlers.```python
import os
import uuid # For generating unique filenames in upload_profile_picture
import json # For SSE
from flask import (
...
# Import queue for SSE related tasks if not already available via current_app or similar
import queue
        ...
        if post_id in current_app.sse_listeners:
            ...
            if listeners:
                sse_post_data = {"id": post.id, "title": post.title, "content": post.content,
                                 "last_edited": post.last_edited.strftime("%Y-%m-%d %H:%M:%S") if post.last_edited else None}
                sse_event = {"type": "post_edited", "payload": sse_post_data}
                for q_item in listeners: # Renamed q to q_item
...
def add_comment(post_id):
    ...
    if post_id in current_app.sse_listeners:
        sse_comment_data = {"id": new_comment_db.id, "author_username": new_comment_db.author.username,
                            "content": new_comment_db.content, "timestamp": new_comment_db.timestamp.strftime("%Y-%m-%d %H:%M:%S")}
        sse_event = {"type": "new_comment", "payload": sse_comment_data}
        for q_item in current_app.sse_listeners[post_id]: # Renamed q to q_item
            try: q_item.put_nowait(sse_event)
            except Exception as e: current_app.logger.error(f"SSE: Error putting new_comment event for post {post_id}: {e}")
...
def post_stream(post_id):
    def event_stream():
        q_local = queue.Queue() # Renamed q to q_local
        if post_id not in current_app.sse_listeners:
            current_app.sse_listeners[post_id] = []
...
def create_event():
    if request.method == "POST":
        ...
        try:
            ...
            db.session.add(activity)
            db.session.commit()
            emit_new_activity_event(activity) # Added emit
        except Exception as e: current_app.logger.error(f"Error creating UserActivity for new_event: {e}"); db.session.rollback()
        flash("Event created successfully!", "success")
        return redirect(url_for("core.events_list"))
    return render_template("create_event.html")
...
def send_friend_request(target_user_id):
    ...
    if sender_user_obj:
        ...
        if target_user_id in current_app.user_notification_queues:
            user_queues = current_app.user_notification_queues[target_user_id]
            if user_queues:
                for q_item in user_queues: # Renamed q to q_item
                    try: q_item.put_nowait(notification_payload); current_app.logger.info(f"Dispatched 'friend_request_received' SSE for user {target_user_id}")
                    except queue.Full: current_app.logger.error(f"SSE queue full for user {target_user_id} for 'friend_request_received'.")
                    except Exception as e: current_app.logger.error(f"Error putting 'friend_request_received' SSE for user {target_user_id}: {e}")
    return redirect(url_for("core.user_profile", username=target_user.username))
