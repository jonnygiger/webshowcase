                        ...
                        for q_item in queues_for_friend:
                            try:
                                ...
                                current_app.logger.info(
                                    f"Dispatched new_activity_event (SSE) to user {friend.id} for activity {activity_log.id}"
                                )
                            except queue.Full:
                                current_app.logger.error(
                                    f"SSE queue full for user {friend.id} (new_activity_event)."
                                )
                            except Exception as e:
                                current_app.logger.error(
                                    f"Error putting new_activity_event to SSE queue for user {friend.id}: {e}"
                                )
                    else:
                        current_app.logger.info(
                            f"No active SSE queues for user {friend.id} (new_activity_event)."
                        )
                else:
                    current_app.logger.info(
                        f"User {friend.id} not in user_notification_queues (new_activity_event)."
                    )
    else:
        current_app.logger.info(
    ...
    if current_user_id_val and current_user_id_val != user.id:
        ...
        if existing_friendship:
            if existing_friendship.status == "accepted":
                ...
                if existing_friendship.user_id == current_user_id_val:
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            if user:
                ...
                image_list = [
                    img.strip() for img in current_images_str.split(",") if img.strip()
                ]
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            file_path = os.path.join(
                current_app.config["PROFILE_PICS_FOLDER"], unique_filename
            )
            try:
                file.save(file_path)
                ...
                if user:
                    ...
                    try:
                        ...
                        db.session.rollback()
                        current_app.logger.error(
                            f"Error creating UserActivity for profile pic update: {e}"
                        )
                    flash("Profile picture uploaded successfully!", "success")
                    return redirect(
                        url_for("core.user_profile", username=user.username)
                    )
            except Exception as e:
...
def edit_profile():
    ...
    if request.method == "POST":
        ...
        if not new_email:
            ...
            return render_template(
                "edit_profile.html",
                user={"username": new_username, "email": user.email, "bio": new_bio},
            )
...
def discover_feed():
    ...
    groups_with_reasons = [
        (g, "Recommended group") for g in recommended_groups_raw if g
    ]
    events_with_reasons = [
        (e, "Recommended event") for e in recommended_events_raw if e
    ]
    return render_template(
        "discover.html",
...
def blog():
    ...
    for post_item in all_posts:
        post_item.review_count = len(post_item.reviews)
        post_item.average_rating = (
            (sum(r.rating for r in post_item.reviews) / len(post_item.reviews))
            if post_item.reviews
            else 0
        )
...
def view_post(post_id):
    ...
    if current_user_id_val:
        user_has_liked = (
            Like.query.filter_by(user_id=current_user_id_val, post_id=post.id).count()
            > 0
        )
    post_reviews = (
    ...
    average_rating = (
        (sum(r.rating for r in post_reviews) / len(post_reviews)) if post_reviews else 0
    )
    can_submit_review = False
    if current_user_id_val:
        is_author = post.user_id == current_user_id_val
        has_reviewed = (
            Review.query.filter_by(user_id=current_user_id_val, post_id=post.id).count()
            > 0
        )
    ...
    if current_user_id_val:
        user_has_bookmarked = (
            Bookmark.query.filter_by(
                user_id=current_user_id_val, post_id=post.id
            ).first()
            is not None
        )
...
def admin_feature_post(post_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error toggling feature status for post {post_id}: {e}"
        )
...
def add_comment(post_id):
    ...
    if post_id in current_app.post_event_listeners:
        ...
        sse_event_for_post_stream = {
            "event": "new_comment",
            "data": comment_data_for_post_stream,
        }
        ...
        for q_item in listeners:
            try:
                ...
                current_app.logger.error(
                    f"SSE queue full for post {post_id} (new_comment on post stream)."
                )
            except Exception as e:
                current_app.logger.error(
                    f"Error putting new_comment to post SSE queue for post {post_id}: {e}"
                )
    else:
        current_app.logger.debug(
                ...
                if queues_for_author:
                    ...
                    for q_item in queues_for_author:
                        try:
                            q_item.put_nowait(sse_event_data)
                            current_app.logger.info(
                                f"Dispatched new_comment_on_post (SSE) to user {post_author_id} for post {post.id}"
                            )
                        except queue.Full:
                            current_app.logger.error(
                                f"SSE queue full for user {post_author_id} (new_comment_on_post)."
                            )
                        except Exception as e:
                            current_app.logger.error(
                                f"Error putting new_comment_on_post to SSE queue for user {post_author_id}: {e}"
                            )
                else:
                    current_app.logger.info(
                        f"No active SSE queues for user {post_author_id} (new_comment_on_post)."
                    )
            else:
                current_app.logger.info(
                    f"User {post_author_id} not in user_notification_queues (new_comment_on_post)."
                )
                ...
                if liker and post.author:
                    ...
                    try:
                        ...
                        if post.author.id in current_app.user_notification_queues:
                            queues_for_author = current_app.user_notification_queues[
                                post.author.id
                            ]
                            if queues_for_author:
                                sse_payload = {
                                ...
                                for q_item in queues_for_author:
                                    try:
                                        q_item.put_nowait(sse_event_data)
                                        current_app.logger.info(
                                            f"Dispatched new_like (SSE) to user {post.author.id} for post {post.id}"
                                        )
                                    except queue.Full:
                                        current_app.logger.error(
                                            f"SSE queue full for user {post.author.id} (new_like)."
                                        )
                                    except Exception as e_sse:
                                        current_app.logger.error(
                                            f"Error putting new_like to SSE queue for user {post.author.id}: {e_sse}"
                                        )
                            else:
                                current_app.logger.info(
                                    f"No active SSE queues for user {post.author.id} (new_like)."
                                )
                        else:
                            current_app.logger.info(
                                f"User {post.author.id} not in user_notification_queues (new_like)."
                            )
                    except Exception as e_notify:
                        db.session.rollback()
                        current_app.logger.error(
                            f"Error creating like notification DB entry: {e_notify}"
                        )
            try:
                activity = UserActivity(
                    user_id=user_id,
                ...
                db.session.rollback()
                current_app.logger.error(
                    f"Error creating UserActivity for new_like: {e_activity}"
                )
    ...
    def event_generator():
        try:
            while True:
                ...
                if data is None:
                    current_app.logger.info(
                        f"Post event stream for post {post_id}, client {client_id_for_log} received None, closing."
                    )
                    break
                ...
                yield sse_message
                current_app.logger.debug(
                    f"Sent SSE event '{event_type}' for post {post_id} to client {client_id_for_log}"
                )

        except GeneratorExit:
            current_app.logger.info(
                f"Post event stream for post {post_id}, client {client_id_for_log} disconnected (GeneratorExit)."
            )
        except Exception as e:
            current_app.logger.error(
                f"Error in post event_generator for post {post_id}, client {client_id_for_log}: {e}",
                exc_info=True,
            )
        finally:
            current_app.logger.info(
...
def chat_stream(room_id):
    ...
    current_app.logger.info(
        f"User {current_user.id if current_user.is_authenticated else 'Unknown'} connected to chat stream for room {room_id}. Active listeners: {len(current_app.chat_room_listeners[room_id])}"
    )
        ...
        try:
            while True:
                ...
                if data is None:  # Sentinel for closing the stream for this client
                    current_app.logger.info(
                        f"SSE stream for room {room_id}, user {current_user.id if current_user.is_authenticated else 'Unknown'} received None, closing."
                    )
                    break
                ...
                yield sse_message
                current_app.logger.debug(
                    f"Sent SSE event '{event_type}' to room {room_id} for user {current_user.id if current_user.is_authenticated else 'Unknown'}"
                )

        except GeneratorExit:
            current_app.logger.info(
                f"SSE stream for room {room_id}, user {current_user.id if current_user.is_authenticated else 'Unknown'} disconnected (GeneratorExit)."
            )
        except Exception as e:
            current_app.logger.error(
                f"Error in SSE event_generator for room {room_id}, user {current_user.id if current_user.is_authenticated else 'Unknown'}: {e}",
                exc_info=True,
            )
        finally:
            current_app.logger.info(
                f"Cleaning up queue for room {room_id}, user {current_user.id if current_user.is_authenticated else 'Unknown'}."
            )
            if (
                room_id in current_app.chat_room_listeners
                and q_local in current_app.chat_room_listeners[room_id]
            ):
                current_app.chat_room_listeners[room_id].remove(q_local)
                if not current_app.chat_room_listeners[
                    room_id
                ]:  # If list is empty, remove key
                    del current_app.chat_room_listeners[room_id]
                    current_app.logger.info(
                        f"Removed room {room_id} from chat_room_listeners as it's empty."
                    )
...
def send_message(receiver_username):
    ...
    if request.method == "POST":
        ...
        new_message_db = Message(
            sender_id=sender_id, receiver_id=receiver_user.id, content=content
        )
        ...
        message_payload = {
            ...
            "timestamp": new_message_db.timestamp.isoformat(),  # Use isoformat
        ...
        if new_message_db.receiver_id in current_app.user_notification_queues:
            queues_for_receiver = current_app.user_notification_queues[
                new_message_db.receiver_id
            ]
            if queues_for_receiver:
                ...
                for q_item in queues_for_receiver:
                    try:
                        q_item.put_nowait(sse_event_dm)
                    except Exception as e:
                        current_app.logger.error(
                            f"SSE: Error putting new_direct_message for user {new_message_db.receiver_id}: {e}"
                        )
        ...
        inbox_update_payload = {
            ...
            "unread_count": unread_count,  # Use isoformat
        ...
        if new_message_db.receiver_id in current_app.user_notification_queues:
            queues_for_receiver_inbox = current_app.user_notification_queues[
                new_message_db.receiver_id
            ]
            if queues_for_receiver_inbox:
                sse_event_inbox = {
                    "type": "update_inbox",
                    "payload": inbox_update_payload,
                }
                for q_item in queues_for_receiver_inbox:
                    try:
                        q_item.put_nowait(sse_event_inbox)
                    except Exception as e:
                        current_app.logger.error(
                            f"SSE: Error putting update_inbox for user {new_message_db.receiver_id}: {e}"
                        )

        flash("Message sent successfully!", "success")
        return redirect(
            url_for("core.view_conversation", username=receiver_user.username)
        )
    return render_template(
        "send_message.html", receiver_username=receiver_user.username
    )
...
def view_conversation(username):
    ...
    relevant_messages = (
        Message.query.filter(
            or_(
                (Message.sender_id == current_user_id_val)
                & (Message.receiver_id == other_user_id),
                (Message.sender_id == other_user_id)
                & (Message.receiver_id == current_user_id_val),
            )
        )
...
def inbox():
    ...
    sent_to_users = (
        ...
        .filter(Message.sender_id == current_user_id_val)
        .distinct()
    )
    received_from_users = (
        db.session.query(Message.sender_id)
        .filter(Message.receiver_id == current_user_id_val)
        .distinct()
    )
    other_user_ids = {user_tuple[0] for user_tuple in sent_to_users}.union(
        {user_tuple[0] for user_tuple in received_from_users}
    )
    inbox_items = []
    for other_id in other_user_ids:
        ...
        unread_count = Message.query.filter_by(
            sender_id=other_id, receiver_id=current_user_id_val, is_read=False
        ).count()
        if last_message:
            snippet = (
    ...
    if request.method == "POST":
        ...
        try:
            activity = UserActivity(
                ...
                link=url_for(
                    "core.view_event", event_id=new_event_db.id, _external=True
                ),
            )
            db.session.add(activity)
            db.session.commit()
            ...
            current_app.logger.error(f"Error creating UserActivity for new_event: {e}")
            db.session.rollback()
...
def view_poll(poll_id):
    ...
    if current_user_id_val:
        existing_vote = PollVote.query.filter_by(
            user_id=current_user_id_val, poll_id=poll.id
        ).first()
        if existing_vote:
            user_has_voted = True
...
def vote_on_poll(poll_id):
    ...
    if not option_to_vote:
        flash("Invalid option for this poll.", "danger")
        return redirect(url_for("core.view_poll", poll_id=poll_id))
...
def delete_poll(poll_id):
    ...
    if poll_to_delete.user_id != user_id:
        flash("Not authorized to delete this poll.", "danger")
        return redirect(url_for("core.view_poll", poll_id=poll_id))
...
def view_event(event_id):
    ...
    if current_user_id_val:
        user_rsvp = EventRSVP.query.filter_by(
            user_id=current_user_id_val, event_id=event.id
        ).first()
        if user_rsvp:
    ...
    is_organizer = (
        current_user_id_val == event.user_id if current_user_id_val else False
    )
    return render_template(
        "view_event.html",
        event=event,
...
def delete_event(event_id):
    ...
    if event_to_delete.user_id != user_id:
        flash("Not authorized to delete this event.", "danger")
        return redirect(url_for("core.view_event", event_id=event_id))
...
def send_friend_request(target_user_id):
    ...
    if not target_user:
        ...
        return redirect(request.referrer or url_for("core.hello_world"))
    if current_user_id_val == target_user_id:
    ...
    is_blocked_by_current_user = UserBlock.query.filter_by(
        blocker_id=current_user_id_val, blocked_id=target_user_id
    ).first()
    ...
    if existing_friendship:
        if existing_friendship.status == "pending":
            ...
            if existing_friendship.friend_id == current_user_id_val:
                flash("You previously rejected a request from this user.", "info")
            else:
    ...
    new_request = Friendship(
        user_id=current_user_id_val, friend_id=target_user_id, status="pending"
    )
    db.session.add(new_request)
    db.session.commit()
    ...
    if sender_user_obj:
        notification_payload = {
            ...
            "payload": {
                "message": f"{sender_user_obj.username} sent you a friend request.",
                "sender_username": sender_user_obj.username,
            ...
            if user_queues:
                for q_item in user_queues:
                    try:
                        ...
                        current_app.logger.info(
                            f"Dispatched 'friend_request_received' SSE for user {target_user_id}"
                        )
                    except queue.Full:
                        current_app.logger.error(
                            f"SSE queue full for user {target_user_id} for 'friend_request_received'."
                        )
                    except Exception as e:
                        current_app.logger.error(
                            f"Error putting 'friend_request_received' SSE for user {target_user_id}: {e}"
                        )
    return redirect(url_for("core.user_profile", username=target_user.username))
...
def view_friend_requests():
    ...
    pending_requests = Friendship.query.filter_by(
        friend_id=current_user_id_val, status="pending"
    ).all()
...
def accept_friend_request(request_id):
    ...
    if friend_request.friend_id != current_user_id_val:
    ...
    if friend_request.status == "pending":
        ...
        if accepting_user_obj:
            notification_payload = {
                "type": "new_follower",
                "payload": {
                    "message": f"{accepting_user_obj.username} accepted your friend request.",
                    "follower_username": accepting_user_obj.username,
                    "profile_link": url_for(
                        "core.user_profile",
                        username=accepting_user_obj.username,
                        _external=True,
                    ),
                },
            }
                ...
                if user_queues:
                    for q_item in user_queues:
                        try:
                            q_item.put_nowait(notification_payload)
                            current_app.logger.info(
                                f"Dispatched 'new_follower' SSE for user {original_sender_id}"
                            )
                        except queue.Full:
                            current_app.logger.error(
                                f"SSE queue full for user {original_sender_id} for 'new_follower'."
                            )
                        except Exception as e:
                            current_app.logger.error(
                                f"Error putting 'new_follower' SSE for user {original_sender_id}: {e}"
                            )
        try:
            activity = UserActivity(
                user_id=current_user_id_val,
            ...
            current_app.logger.error(f"Error creating UserActivity for new_follow: {e}")
            db.session.rollback()
        return redirect(
            url_for("core.user_profile", username=friend_request.requester.username)
        )
    elif friend_request.status == "accepted":
        flash("You are already friends.", "info")
    else:
        flash("Request no longer pending.", "warning")
    return redirect(url_for("core.view_friend_requests"))
...
def remove_friend(friend_user_id):
    ...
    if friendship_to_remove:
        ...
        db.session.commit()
        flash(f"No longer friends with {friend_user.username}.", "success")
    else:
        flash(f"Not currently friends with {friend_user.username}.", "info")
    return redirect(url_for("core.user_profile", username=friend_user.username))
...
def unblock_user(username_to_unblock):
    ...
    if not user_to_unblock:
        flash("User not found.", "danger")
        return redirect(request.referrer or url_for("core.hello_world"))
    if current_user_id_val == user_to_unblock.id:
        flash("Cannot unblock yourself.", "warning")
        return redirect(url_for("core.user_profile", username=username_to_unblock))
    block_instance = UserBlock.query.filter_by(
        blocker_id=current_user_id_val, blocked_id=user_to_unblock.id
    ).first()
    if block_instance:
        db.session.delete(block_instance)
        db.session.commit()
        flash(f"Unblocked {username_to_unblock}.", "success")
    else:
...
def block_user_route(username_to_block):
    ...
    if not user_to_block:
        flash("User not found.", "danger")
        return redirect(request.referrer or url_for("core.hello_world"))
    if current_user_id_val == user_to_block.id:
    ...
    existing_block = UserBlock.query.filter_by(
        blocker_id=current_user_id_val, blocked_id=user_to_block.id
    ).first()
    if existing_block:
        flash(f"Already blocked {username_to_block}.", "info")
    else:
        friendship_to_remove = Friendship.query.filter(
            or_(
                (Friendship.user_id == current_user_id_val)
...
def edit_series(series_id):
    ...
    if series.user_id != current_user.id:
        flash("Not authorized to edit this series.", "danger")
...
def reorder_series_posts(series_id):
    ...
    if set(new_post_ids_order) != current_post_ids_in_series:
        return jsonify({"status": "error", "message": "Post IDs mismatch."}), 400
    try:
            ...
            if series_post_entry:
                ...
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": f"Error finding post ID {post_id}.",
                        }
                    ),
                    500,
                )
        ...
        db.session.rollback()
        current_app.logger.error(f"Error reordering posts for series {series_id}: {e}")
...
def recommendations_view():
    ...
    return render_template(
        ...
        suggested_hashtags=suggested_hashtags_list,
    )
...
def flag_post(post_id):
    ...
    user_id = current_user.id
    if post.user_id == user_id:
        flash("Cannot flag your own post.", "warning")
        return redirect(url_for("core.view_post", post_id=post_id))
    ...
    if existing_flag:
        flash("Already flagged this post.", "info")
    else:
...
def flag_comment(comment_id):
    ...
    if comment.user_id == user_id:
        flash("Cannot flag your own comment.", "warning")
    ...
    if existing_flag:
        flash("Already flagged this comment.", "info")
    else:
        new_flag = FlaggedContent(
...
def approve_flagged_content(flag_id):
    ...
    if flag.status != "pending":
        flash("Flag already processed.", "warning")
        return redirect(url_for("core.moderation_dashboard"))
    flag.status = "approved"
    flag.moderator_id = current_user.id
    flag.moderator_comment = request.form.get("moderator_comment")
    flag.resolved_at = datetime.now(timezone.utc)
    db.session.commit()
    flash(f"Flag ID {flag.id} approved.", "success")
    return redirect(url_for("core.moderation_dashboard"))
...
def reject_flagged_content(flag_id):
    ...
    flag.resolved_at = datetime.now(timezone.utc)
    db.session.commit()
    flash(f"Flag ID {flag.id} rejected.", "success")
    return redirect(url_for("core.moderation_dashboard"))


@core_bp.route(
    "/flagged_content/<int:flag_id>/remove_content_and_reject", methods=["POST"]
)
@login_required
@moderator_required
def remove_content_and_reject_flag(flag_id):
    ...
    if flag.content_type == "post":
        ...
        if post_to_delete:
            ...
            flash(f"Post ID {flag.content_id} deleted.", "info")
        else:
            flash(f"Post ID {flag.content_id} not found.", "error")
    elif flag.content_type == "comment":
        comment_to_delete = db.session.get(Comment, flag.content_id)
        if comment_to_delete:
            db.session.delete(comment_to_delete)
            content_removed = True
            flash(f"Comment ID {flag.content_id} deleted.", "info")
        else:
            flash(f"Comment ID {flag.content_id} not found.", "error")
    else:
        flash(f"Unsupported content type '{flag.content_type}'.", "error")
...
def mark_friend_post_notification_as_read(notification_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error marking friend post notification as read: {e}")
        return jsonify({"status": "error", "message": "Could not mark as read."}), 500
...
def mark_all_friend_post_notifications_as_read():
    ...
    try:
        ...
        if not unread_notifications:
            return jsonify({"status": "success", "message": "No unread notifications."})
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error marking all friend post notifications as read: {e}"
        )
        return (
            jsonify({"status": "error", "message": "Could not mark all as read."}),
            500,
        )
    ...
    if request.method == "POST":
        ...
        if file and allowed_shared_file(file.filename):
            ...
            try:
                ...
                db.session.rollback()
                current_app.logger.error(f"Error saving/sharing file: {e}")
...
def files_inbox():
    received_files = (
        SharedFile.query.filter_by(receiver_id=current_user.id)
...
def delete_shared_file(shared_file_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error deleting shared file: {e}")
...
def user_notification_stream():
    ...
    current_app.logger.info(
        f"User {current_user_id_val} connected to notification stream. Queues: {len(current_app.user_notification_queues[current_user_id_val])}"
    )
    ...
    def event_stream():
        try:
            while True:
                ...
                if data is None:
                    current_app.logger.info(
                        f"Stream for user {current_user_id_val} closing."
                    )
                    break
                event_type = data.get("type", "message")
                payload = data.get("payload", {})
                ...
                yield sse_message
                current_app.logger.debug(
                    f"Sent event {event_type} to user {current_user_id_val}"
                )
        except GeneratorExit:
            current_app.logger.info(
                f"User {current_user_id_val} disconnected (GeneratorExit)."
            )
        except Exception as e:
            current_app.logger.error(
                f"Error in event stream for user {current_user_id_val}: {e}"
            )
        finally:
            current_app.logger.info(
                f"Cleaning up queue for user {current_user_id_val}."
            )
            if current_user_id_val in current_app.user_notification_queues:
                if q_local in current_app.user_notification_queues[current_user_id_val]:
                    current_app.user_notification_queues[current_user_id_val].remove(
                        q_local
                    )
