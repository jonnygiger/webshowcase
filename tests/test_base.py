...
from models import User, Message, Post, Friendship, FriendPostNotification, Group, Event, Poll, PollOption, TrendingHashtag, SharedFile, UserStatus, Achievement, UserAchievement, Comment, Series, SeriesPost, Notification, Like, EventRSVP, PollVote, PostLock # COMMENTED OUT - Added EventRSVP, PollVote, PostLock
...
class AppTestCase(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        shared_files_folder = app.config.get('SHARED_FILES_UPLOAD_FOLDER') # temp replacement for app.config.get('SHARED_FILES_UPLOAD_FOLDER')
    ...
    def _create_db_message(self, sender_id, receiver_id, content, timestamp=None, is_read=False):
        ...
        msg = Message(sender_id=sender_id, receiver_id=receiver_id, content=content, timestamp=timestamp or datetime.utcnow(), is_read=is_read)
        db.session.add(msg)
        db.session.commit()
        return msg
    ...
    def _get_jwt_token(self, username, password):
        response = self.client.post('/login_api', json={'username': username, 'password': password})
    ...
    def _create_db_event(self, user_id, title="Test Event", description="An event for testing", date_str="2024-12-31", time="18:00", location="Test Location", created_at=None):
        # Requires live Event model and db session
        event_datetime = datetime.strptime(f"{date_str} {time}", "%Y-%m-%d %H:%M")
        event = Event(user_id=user_id, title=title, description=description, date=event_datetime, location=location, created_at=created_at or datetime.utcnow())
    ...
    def _create_db_poll(self, user_id, question="Test Poll?", options_texts=None, created_at=None):
        ...
        poll = Poll(user_id=user_id, question=question, created_at=created_at or datetime.utcnow())
        db.session.add(poll)
    ...
    def _create_db_poll_vote(self, user_id, poll_id, poll_option_id, created_at=None):
        ...
        vote = PollVote(user_id=user_id, poll_id=poll_id, poll_option_id=poll_option_id, created_at=created_at or datetime.utcnow())
        db.session.add(vote)
        db.session.commit()
        return vote
    ...
    def _create_series(self, user_id, title="Test Series", description="A series for testing.", created_at=None, updated_at=None):
        ...
        series = Series(user_id=user_id, title=title, description=description, created_at=created_at or datetime.utcnow(), updated_at=updated_at or datetime.utcnow())
