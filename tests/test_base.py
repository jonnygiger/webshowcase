...
from models import (
    User, Message, Post, Friendship, FriendPostNotification, Group, Event,
    Poll, PollOption, TrendingHashtag, SharedFile, UserStatus, Achievement,
    UserAchievement, Comment, Series, SeriesPost, Notification, Like,
    EventRSVP, PollVote, PostLock,
)
...
class AppTestCase(unittest.TestCase):
    ...
    def login(self, username, password, client_instance=None):
            ...
            if hasattr(self.app, 'session_interface') and hasattr(flask, 'session'):
                 ...
                 for header_name, header_value in dummy_response.headers:
                     if header_name == 'Set-Cookie' and 'session=' in header_value:
                         try:
                             cookie_full_str = header_value.split(';')[0]
                             if 'session=' in cookie_full_str:
                                 session_cookie_value = cookie_full_str.split('session=')[1]
                                 print(f"DEBUG: Extracted session cookie from dummy_response for {username}: {session_cookie_value[:20]}...", file=sys.stderr)
                                 break
                         except IndexError:
        ...
        if session_cookie_value:
            ...
            print(f"DEBUG: Using explicit 'Cookie' header for SocketIO connect for {username}: session={session_cookie_value[:20]}...", file=sys.stderr)
        else:
        ...
        retry_count = 0
        max_retries = 20
        while not getattr(socketio_client_to_use, 'sid', None) and retry_count < max_retries:
            time.sleep(0.1)
            retry_count += 1
    ...
    def logout(self, client_instance=None):
        http_client = self.client
        socket_client_to_disconnect = client_instance if client_instance else getattr(self, "socketio_client", None)
    ...
    def _create_db_message(self, sender_id, receiver_id, content, timestamp=None, is_read=False):
        with self.app.app_context():
            msg = Message(sender_id=sender_id, receiver_id=receiver_id, content=content, timestamp=timestamp or datetime.now(timezone.utc), is_read=is_read)
    ...
    def _get_jwt_token(self, username, password):
        ...
        self.assertEqual(response.status_code, 200, f"Login failed for token generation: {response.data.decode()}")
    ...
    def _create_db_event(self, user_id, title="Test Event", description="An event for testing", date_str="2024-12-31", time_str="18:00", location="Test Location", created_at=None):
        with self.app.app_context():
            event_datetime_obj = datetime.strptime(f"{date_str} {time_str}", "%Y-%m-%d %H:%M")
            event = Event(user_id=user_id, title=title, description=description, date=event_datetime_obj, location=location, created_at=created_at or datetime.now(timezone.utc))
    ...
    def _create_db_poll(self, user_id, question="Test Poll?", options_texts=None, created_at=None):
        if options_texts is None: options_texts = ["Option 1", "Option 2"]
        with self.app.app_context():
            poll = Poll(user_id=user_id, question=question, created_at=created_at or datetime.now(timezone.utc))
    ...
    def _create_db_like(self, user_id, post_id, timestamp=None):
        ...
        with self.app.app_context():
            like = Like(user_id=user_id, post_id=post_id, timestamp=timestamp or datetime.now(timezone.utc))
    ...
    def _create_db_comment(self, user_id, post_id, content="Test comment", timestamp=None):
        ...
        with self.app.app_context():
            comment = Comment(user_id=user_id, post_id=post_id, content=content, timestamp=timestamp or datetime.now(timezone.utc))
    ...
    def _create_db_event_rsvp(self, user_id, event_id, status="Attending", timestamp=None):
        ...
        with self.app.app_context():
            rsvp = EventRSVP(user_id=user_id, event_id=event_id, status=status, timestamp=timestamp or datetime.now(timezone.utc))
    ...
    def _create_db_poll_vote(self, user_id, poll_id, poll_option_id, created_at=None):
        ...
        with self.app.app_context():
            vote = PollVote(user_id=user_id, poll_id=poll_id, poll_option_id=poll_option_id, created_at=created_at or datetime.now(timezone.utc))
            self.db.session.add(vote)
            self.db.session.commit()
            return vote.id

    def _create_series(self, user_id, title="Test Series", description="A series for testing.", created_at=None, updated_at=None): # This seems to be a duplicate of _create_db_series
        with self.app.app_context():
            series = Series(user_id=user_id, title=title, description=description, created_at=created_at or datetime.now(timezone.utc), updated_at=updated_at or datetime.now(timezone.utc))
    ...
    def _create_db_bookmark(self, user_id, post_id, timestamp=None):
        ...
        with self.app.app_context():
            bookmark = Bookmark(user_id=user_id, post_id=post_id, timestamp=timestamp or datetime.now(timezone.utc))
    ...
    def _create_db_block(self, blocker_user_obj, blocked_user_obj, timestamp=None):
        ...
        with self.app.app_context():
            block = UserBlock(blocker_id=blocker_user_obj.id, blocked_id=blocked_user_obj.id, timestamp=timestamp or datetime.now(timezone.utc))
    ...
    def _create_db_friendship(self, user_obj_1, user_obj_2, status="accepted", timestamp=None):
        with self.app.app_context():
            friendship = Friendship(user_id=user_obj_1.id, friend_id=user_obj_2.id, status=status, timestamp=timestamp or datetime.now(timezone.utc))
