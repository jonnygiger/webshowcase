...
class ChatTestCase(AppTestCase):
    ...
    def test_get_chat_room_messages_api(self):
        with self.app.app_context():
            ...
            msg1 = ChatMessage(room_id=room.id, user_id=self.user1_id, message="Hello from user1", timestamp=msg1_time)
            db.session.add(msg1)
            db.session.commit()
            ...
            response = self.client.get(f'/api/chat/rooms/{room.id}/messages',
                                       headers={'Authorization': f'Bearer {self.user1_token}'})
            self.assertEqual(response.status_code, 200)
            data = response.get_json()
            self.assertIn('messages', data)
            self.assertEqual(len(data['messages']), 1) # Adjusted to 1 message for simplicity
    ...
    def test_socketio_join_and_send_message(self):
        with self.app.app_context():
            ...
            new_message_events = [r for r in received if r['name'] == 'new_chat_message']
            self.assertTrue(len(new_message_events) > 0, "No 'new_chat_message' event received")
            ...
            for event_data in new_message_events:
                args = event_data['args'][0] # Assuming message payload is the first argument
                if args['message'] == test_message and args['username'] == self.user1.username:
                    found_message = True
                    self.assertEqual(args['room_name'], socket_room_name)
                    self.assertEqual(args['user_id'], self.user1_id)
                    break
            self.assertTrue(found_message, "Sent message not found in received SocketIO events")
    ...
    def test_chat_page_redirects_for_anonymous_user(self):
        response = self.client.get('/chat', follow_redirects=False) # Important: follow_redirects=False
