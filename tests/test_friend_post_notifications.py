...
class TestFriendPostNotifications(AppTestCase):  # Inherit from AppTestCase for setup
    ...
    def test_view_friend_post_notifications_page(self):
        with self.app.app_context():
            ...
            self.assertIsNotNone(
                post1_obj_by_user1, "Post1 object by User1 should not be None."
            )
            ...
            self.assertIsNotNone(
                post2_obj_by_user3, "Post2 object by User3 should not be None."
            )
            ...
            self.assertIn(
                self.user3.username, response_data
            )  # Poster of newer notification
            self.assertIn(post2_obj_by_user3.title, response_data)
            self.assertIn(
                self.user1.username, response_data
            )  # Poster of older notification
    ...
    def test_mark_all_notifications_as_read(self):
        with self.app.app_context():
            ...
            )  # User3's notification untouched
            self.logout()
    ...
    def test_no_notification_for_own_post(self, mock_socketio_emit):
        with self.app.app_context():
            ...
            self.assertIsNotNone(
                created_post, "Post creation failed or post not found."
            )
            ...
            self.assertIsNone(
                notification_for_self, "A notification was created for user's own post."
            )
            ...
            for call in mock_socketio_emit.call_args_list:
                ...
                if event_name == "new_friend_post" and room == f"user_{self.user1_id}":
                    ...
                    if "post_id" in payload and payload["post_id"] == created_post.id:
                        self.fail(
                            f"socketio.emit called for user's own post: event={event_name}, room={room}, payload={payload}"
                        )
    ...
    def test_no_notification_for_post_before_friendship(self, mock_socketio_emit):
        with self.app.app_context():
            ...
            notification_for_user2 = FriendPostNotification.query.filter_by(
                user_id=self.user2_id, post_id=created_post.id  # or post_id variable
            ).first()
            self.assertIsNone(
                notification_for_user2,
    ...
    def test_notification_persists_after_unfriend(self, mock_socketio_emit_unfriend):
        with self.app.app_context():
            ...
            self.assertIsNotNone(
                notification_for_b,
                "Notification for User B should exist after User A posts.",
            )
            self.assertFalse(
                notification_for_b.is_read,
                "Notification for User B should initially be unread.",
            )
            ...
            friendship_record = Friendship.query.filter(
                ...
                Friendship.status
                == "accepted",  # Ensure we are targeting the active friendship
            ).first()

            self.assertIsNotNone(
                friendship_record, "Friendship record should exist before unfriending."
            )
            ...
            self.assertIsNone(
                deleted_friendship_record,
                "Friendship record should be deleted after unfriending.",
            )
