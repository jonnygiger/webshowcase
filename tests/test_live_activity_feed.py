import unittest
from unittest.mock import patch, call, ANY, MagicMock # Added MagicMock here as it's used in helper
...
def _create_db_user_activity(user_id, activity_type, related_id=None, target_user_id=None, content_preview=None, link=None, timestamp=None):
    ...
    return {
        ...
        'target_user': mock_target_user if target_user_id else None # Mocked target_user object
    }
...
class TestLiveActivityFeed(AppTestCase):
    ...
    @patch('app.socketio.emit') # Assuming 'app.socketio.emit' is the correct path to mock
