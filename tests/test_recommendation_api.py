...
from models import (
    ...
    EventRSVP,
    PollVote,
)  # Ensure this line is active or add necessary imports
...
class TestRecommendationAPI(AppTestCase):
    ...
    def test_recommend_group_joined_by_friend(self):
        ...
        with self.app.app_context():  # Add app context here
            ...
            group_by_user3_merged = self.db.session.merge(
                group_by_user3
            )  # Ensure group is in session
            ...
            if user2 and group_by_user3_merged:
                ...
                self.db.session.add(
                    user2
                )  # Add user2 to session if relationship change doesn't auto-add
