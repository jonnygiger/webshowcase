...
class TestSuggestTrendingPosts(AppTestCase):
    ...
    def test_no_posts_returns_empty_list(self):
        ...
        with self.app.app_context():
            trending_posts = suggest_trending_posts(user_id=self.user1.id)
    ...
    def test_since_days_parameter(self):
        ...
        with self.app.app_context():
            ...
            post_old_old_like = self._create_post(
                self.user3, self.now - timedelta(days=15)
            )
            self._create_like(
                self.user2, post_old_old_like, self.now - timedelta(days=10)
            )
