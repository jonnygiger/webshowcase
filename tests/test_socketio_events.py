...
class TestSocketIOEvents(AppTestCase):
    ...
    def test_socketio_new_like_notification(self):
        with self.app.app_context():
            ...
            author_socket_client = self.socketio_class_level.test_client(
                self.app
            )  # No flask_test_client linkage
            ...
            time.sleep(
                0.5
            )  # Added delay: Allow server to fully process the connection and session from login

            # Author's client joins its room
            author_socket_client.emit(
                "join_room", {"room": f"user_{post_author.id}"}, namespace="/"
            )
            ...
            while (
                time.time() - notification_receive_start_time < 1.0
            ):  # Collect for up to 1 second
                ...
                if received_event_batch:
                    ...
                    if like_notification_events := [
                        r
                        for r in received_by_author_total
                        if r["name"] == "new_like_notification"
                    ]:
    ...
    def test_socketio_post_lock_released_on_manual_release(self):
        with self.app.app_context():
            ...
            listener_client.emit(
                "join_room", {"room": f"post_{post_to_lock.id}"}, namespace="/"
            )
            # Clear any ack from join_room
            join_ack_start_time = time.time()
            while time.time() - join_ack_start_time < 0.2:  # Max 0.2s to clear
            ...
            lock_released_events = [
                r for r in received_by_listener if r["name"] == "post_lock_released"
            ]

            self.assertTrue(
                len(lock_released_events) > 0,
                "Listener did not receive 'post_lock_released' event",
            )
