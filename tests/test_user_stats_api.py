...
class TestUserStatsAPI(AppTestCase):
    ...
    def test_user_stats_api(self):
        # with app.app_context(): # Handled by test client
            ...
            response = self.client.get(f'/api/users/{self.user1_id}/stats', headers=headers_user1)
            ...
            response_forbidden = self.client.get(f'/api/users/{self.user1_id}/stats', headers=headers_user2)
