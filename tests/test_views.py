...
class TestViewRoutes(AppTestCase):
    ...
    def test_user_profile_friendship_status_display(self):
        with self.app.app_context():
            ...
            self.assertIn(url_for('core.accept_friend_request', request_id=fs_b_to_a.id), response.data.decode())
            self.logout()
    ...
    def test_remove_friend_functionality(self):
        with self.app.app_context():
            ...
            fs_check = Friendship.query.filter_by(user_id=user_x.id, friend_id=user_y.id, status="accepted").first()
            fs_check_reverse = Friendship.query.filter_by(user_id=user_y.id, friend_id=user_x.id, status="accepted").first()
    ...
    def test_file_sharing_download_authorization(self):
        with self.app.app_context():
            ...
            response_other = self.client.get(url_for('core.download_shared_file', shared_file_id=shared_file.id), follow_redirects=False)
