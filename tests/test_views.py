...
class TestViewRoutes(AppTestCase):
    ...
    def test_user_profile_friendship_status_display(self):
        with self.app.app_context():
            ...
            )  # Corrected to match button text
            # Check that the form to accept/reject is present
            self.assertIn(
                f"/friend_request/{fs_b_to_a.id}/accept", response.data.decode()
            )
            self.logout()
    ...
    def test_moderation_dashboard_access(self):
        with self.app.app_context():
            ...
            )  # Should redirect to login or home if already logged in but not authorized
    ...
    def test_remove_friend_functionality(self):
        with self.app.app_context():
            ...
            response = self.client.post(
                f"/user/{user_y.id}/remove_friend", follow_redirects=True
            )
            ...
            fs_check = Friendship.query.filter_by(
                user_id=user_x.id, friend_id=user_y.id
            ).first()
            fs_check_reverse = Friendship.query.filter_by(
                user_id=user_y.id, friend_id=user_x.id
            ).first()
    ...
    def test_file_sharing_download_authorization(self):
        with self.app.app_context():
            ...
            saved_filename_on_disk = (
                "uuid_test_auth_file.txt"  # Needs to be unique if other tests use it
            )
            ...
            response_other = self.client.get(
                f"/files/download/{shared_file.id}", follow_redirects=False
            )
    ...
    def test_user_blocking_effects_on_profile_and_posts(self):
        with self.app.app_context():
            ...
            )  # Current state: profile likely visible
            ...
            self.assertEqual(
                response_post_direct.status_code, 200
            )  # Current state: post likely visible
